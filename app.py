...
def reshape_long(df: pd.DataFrame, id_col: str, lat_col: Optional[str], lon_col: Optional[str], zone_col: Optional[str]) -> Tuple[pd.DataFrame, List[int]]:
    ...
    long = work.melt(id_vars=[c for c in [id_col, lat_col, lon_col, zone_col] if c],
...
# Re-ordered tabs: Report View, Map, then Profiles & Averages
...
# Title / Field name prompt after upload (visible in main area)
...
# -------------------------
# -------------------------
# Tab 1 (now): Report View (distinct look)
# -------------------------
with tabs[0]:
    ...
    for name, (lo, hi) in intervals.items():
        ...
        top_rows.append({"Interval": name, "Highest_PSI": m["PSI"].max()})
    ...
    with c2:
        st.metric("Highest PSI (0–3 in)", f"{top_df.loc[top_df['Interval']=='0–3 in','Highest_PSI'].values[0]:.0f}")
        st.metric("Highest PSI (4–7 in)", f"{top_df.loc[top_df['Interval']=='4–7 in','Highest_PSI'].values[0]:.0f}")
        st.metric("Highest PSI (8–11 in)", f"{top_df.loc[top_df['Interval']=='8–11 in','Highest_PSI'].values[0]:.0f}")
        st.metric(f"% points >{PSI_THRESHOLDS['high']} PSI (0–11)", f"{exceed:.1f}%")
        st.metric("Overall Compaction Rating", rating)
    with c1:
        # Field Average by Interval (moved here)
...
# -------------------------
# Tab 2 (now): Map + KML export
# -------------------------
with tabs[1]:
    st.subheader("Compaction Map (Interpolated)")
    if lat_col and lon_col:
        # Build an interval-averaged dataset for mapping
        map_interval = st.selectbox("Depth interval", options=["0–3 in", "4–7 in", "8–11 in"], index=0)
        lo, hi = {"0–3 in": (0,3), "4–7 in": (4,7), "8–11 in": (8,11)}[map_interval]
        ...
        # Scatter points (colored by band)
        ...
        # Interpolated heatmap (KDE-style) as a smooth surface
        heat = pdk.Layer(
            "HeatmapLayer",
            data=avg_map,
            get_position='[lon, lat]',
        ...
        try:
            ...
            st.download_button("⬇️ Download interval average (KML)", data=kml_bytes, file_name=f"compaction_{map_interval.replace(' ','').replace('–','-')}.kml", mime="application/vnd.google-earth.kml+xml")
        except Exception:
...
# -------------------------
# Tab 3 (now): Profiles & Averages (per-point table etc.)
# -------------------------
with tabs[2]:
    ...
    st.download_button("⬇️ Download selection @ depth (CSV)", data=selcsv, file_name=f"compaction_depth_{depth_sel}in.csv", mime="text/csv") (distinct look)
# -------------------------
with tabs[1]:
    st.subheader("Field Summary")
    # Highest resistance 0–6 and 7–11
    ...
    # Overall rating based on exceedance
    def overall_rating(x):
        if x >= 50:
            return "Severe"
        if x >= 20:
    ...
    with c2:
        ...
        st.metric("Overall Compaction Rating", rating)
    with c1:
        band_df = field_avg.copy()
        fig_rep = px.area(band_df, x="Interval", y="PSI_mean", title="Average PSI by Interval")
        fig_rep.add_hline(y=PSI_THRESHOLDS["high"], line_dash="dash", line_color=PSI_COLORS_HEX["high"])
        fig_rep.add_hline(y=PSI_THRESHOLDS["moderate"], line_dash="dash", line_color=PSI_COLORS_HEX["moderate"])
        fig_rep.add_hline(y=PSI_THRESHOLDS["low"], line_dash="dash", line_color=PSI_COLORS_HEX["low"])
        st.plotly_chart(fig_rep, use_container_width=True)
...
# -------------------------
# Tab 3: Map (checkbox per depth) + KML export
# -------------------------
with tabs[2]:
    ...
    if lat_col and lon_col:
        ...
        layers = []
        for d in sel_depths:
            layer_df = long[(long["Depth_in"] == d) & long[[lat_col, lon_col]].notna().all(axis=1)] \
                [[id_col, lat_col, lon_col, "PSI"]] \
        ...
        if layers:
            ...
            deck = pdk.Deck(layers=layers, initial_view_state=view)  # default basemap
            st.pydeck_chart(deck)
        else:
        ...
        try:
            ...
            for d in sel_depths:
                ...
                for _, r in rows.iterrows():
                    p = folder.newpoint(name=f"{id_col}: {r[id_col]} | {r['PSI']:.0f} PSI",
                                        coords=[(float(r[lon_col]), float(r[lat_col]))])
                    p.style.iconstyle.color = psi_kml_color(float(r["PSI"]))
                    p.style.iconstyle.scale = 0.8
            kml_bytes = kml.kml().encode("utf-8")
